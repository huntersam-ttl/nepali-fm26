...
HEAD `290dd54`; the pre-existing untracked `nepali-fm26/` entry
remains untouched.
...
- Chairman/owner career; federation-president career; broader role transitions, authority, and permissions (bounded active-role switching, approved-proposal implementation, chairman budget setting, infrastructure approval, and sponsorship approval are now production-wired).
...
## Final Reconciliation (HEAD `290dd54`)
...
The 15 PARTIAL rows are: chairman/owner career (including bounded budget setting, infrastructure approval, and sponsorship approval, but not a complete chairman command surface); federation-president career (including the
