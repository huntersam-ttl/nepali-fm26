...
const mapState = (row: any): MacroEconomicState => ({
  ...
  inflationIndex: row.inflation_index,
  wageInflationIndex: row.wage_inflation_index,
  playerValueIndex: row.player_value_index,
  landPriceIndex: row.land_price_index,
  constructionCostIndex: row.construction_cost_index,
  sponsorMarketStrength: row.sponsor_market_strength,
  broadcastMarketStrength: row.broadcast_market_strength,
  footballCommercialStrength: row.football_commercial_strength,
  exchangeRateToNpr: row.exchange_rate_to_npr,
  status: row.status,
});
...
export class MacroEconomyRepository {
  constructor(private readonly db: GameDatabase) {
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS macro_economy_progress (
      id INTEGER PRIMARY KEY CHECK (id = 1), last_processed_period TEXT NOT NULL
    )`);
  }
  upsert(state: MacroEconomicState): void {
    this.db
      .prepare(
        `INSERT INTO macro_economic_states
      ...
      .run(
        ...
        state.wageInflationIndex,
        state.playerValueIndex,
        state.landPriceIndex,
        state.constructionCostIndex,
        state.sponsorMarketStrength,
        state.broadcastMarketStrength,
        state.footballCommercialStrength,
        state.exchangeRateToNpr,
        state.status,
      );
  }
  state(countryId: EntityId, year?: number): MacroEconomicState | undefined {
    const row = (
      year === undefined
        ? this.db
            .prepare(
              "SELECT * FROM macro_economic_states WHERE country_id=? ORDER BY year DESC LIMIT 1",
            )
            .get(countryId)
        : this.db
            .prepare("SELECT * FROM macro_economic_states WHERE country_id=? AND year=?")
            .get(countryId, year)
    ) as any;
  ...
  states(countryId?: EntityId): MacroEconomicState[] {
    const rows = (
      countryId
        ? this.db
            .prepare("SELECT * FROM macro_economic_states WHERE country_id=? ORDER BY year")
            .all(countryId)
        : this.db.prepare("SELECT * FROM macro_economic_states ORDER BY country_id,year").all()
    ) as any[];
    return rows.map(mapState);
  }
  lastProcessedPeriod(): string | undefined {
    const row = this.db
      .prepare("SELECT last_processed_period FROM macro_economy_progress WHERE id=1")
      .get() as { last_processed_period?: string } | undefined;
    return row?.last_processed_period;
  }
  markProcessed(period: string): void {
    this.db
      .prepare(
        `INSERT INTO macro_economy_progress (id,last_processed_period) VALUES (1,?)
      ON CONFLICT(id) DO UPDATE SET last_processed_period=excluded.last_processed_period`,
      )
      .run(period);
  }
}
