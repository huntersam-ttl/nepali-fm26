    ...
    this.db
      .prepare(
        `INSERT INTO ownership_acquisition_offers
          (id,club_id,buyer_person_id,seller_holder_id,percentage,offer_amount,counter_amount,status,created_on,decided_on,rationale,investor_type,respond_by,pending_decision_by,deal_structure,owner_proceeds_amount,capital_injection_amount,due_diligence_findings_json,board_stance,provenance_status)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
