...
export const simulateMatch = (input: SimulateMatchInput): MatchResult => {
  ...
  const home: RuntimeTeam = createRuntimeTeam(input.fixture.homeTeamId, homeSelection, true);
  const away: RuntimeTeam = createRuntimeTeam(input.fixture.awayTeamId, awaySelection, false);
  ...
  for (let minute = 1; minute <= 90; minute += 1) {
    ...
    if (rng.next() < attackingSequenceChance(attacking, defending)) {
      ...
      const xg = calculateShotXg(shooter, assister, attacking.strength, defending.strength, rng);
      ...
      const onTargetChance = clamp(
        0.28 + xg + shooter.attributes.technical.finishing / 80,
        0.15,
        0.82,
      );
    ...
    if (rng.next() < foulChance(home, away)) {
    ...
    if (rng.next() < injuryChance(home, away)) {
...
const createRuntimeTeam = (
  ...
  teamId,
  selection,
  strength: calculateTeamStrength({ selection, homeAdvantage }),
  stats: {
    teamId,
    possession: 50,
    shots: 0,
...
const attackingSequenceChance = (attacking: RuntimeTeam, defending: RuntimeTeam): number =>
  clamp(0.12 + (attacking.strength.attack - defending.strength.defense) / 250, 0.07, 0.22);
...
const calculateShotXg = (
  ...
  const chanceBase = rng.next() < 0.16 ? 0.2 : rng.next() < 0.45 ? 0.105 : 0.045;
  ...
  return clamp(
    chanceBase + finisher / 420 + creation / 620 + attacking.attack / 520 - pressure / 700,
    0.015,
    0.55,
  );
};
...
const foulChance = (home: RuntimeTeam, away: RuntimeTeam): number =>
  clamp(0.19 + (home.strength.defense + away.strength.defense) / 900, 0.15, 0.25);

const injuryChance = (home: RuntimeTeam, away: RuntimeTeam): number =>
  clamp(
    0.0035 + (100 - averageFitness(home.states) + 100 - averageFitness(away.states)) / 70000,
    0.002,
    0.009,
  );
