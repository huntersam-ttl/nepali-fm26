...
/**
 * The club-level reconciliation above correctly maps an existing Nepal club
 * to its real, existing `clubs` row rather than duplicating it — but a
 * MAPPED club (one this Nepal world already had, with its own real senior
 * men's team from `import-nepal-world`) must reuse that EXISTING team too.
 * Without this, every Nepal club that also appears in the global dataset
 * (continental/federation context rows) received a SECOND senior-men team
 * row under a separate id scheme, alongside its real domestic team —
 * silently breaking anything that resolves "the club's senior team" by an
 * unscoped query (fixture/matchday lookups, squad views, ...).
 */
const canonicalTeam = (
  db: GameDatabase,
  externalId: string,
  clubId: EntityId,
): { id: EntityId; action: "NEW" | "MAPPED" } => {
  const existingByExternal = db
    .prepare("SELECT id FROM teams WHERE canonical_external_id=? LIMIT 1")
    .get(`${externalId}-MEN`) as { id?: EntityId } | undefined;
  if (existingByExternal?.id) return { id: existingByExternal.id, action: "MAPPED" };
  const existingSenior = db
    .prepare("SELECT id FROM teams WHERE club_id=? AND level='senior' AND gender='men' LIMIT 1")
    .get(clubId) as { id?: EntityId } | undefined;
  return existingSenior?.id
    ? { id: existingSenior.id, action: "MAPPED" }
    : { id: createStableEntityId("import-team", externalId), action: "NEW" };
};
...
export const applyGlobalFootballImportToDatabase = (db: GameDatabase, plan: GlobalImportPlan): GlobalImportApplyCounts => {
  ...
  try {
    ...
    for (const row of plan.clubs) { const external = value(row, "club_external_id"), clubId = clubs.get(external)!, federationId = federations.get(value(row, "national_federation")) ?? findFederation(db, value(row, "national_federation"), country(value(row, "country"))), leagueId = leagueNames.get(key(value(row, "league"))); const resolvedTeam = canonicalTeam(db, external, clubId); teams.set(external, resolvedTeam.id); if (resolvedTeam.action === "NEW") { world.insertTeam({ id: resolvedTeam.id, clubId, federationId, name: `${value(row, "common_name") || value(row, "official_name")} Senior Men`, canonicalExternalId: `${external}-MEN`, level: "senior", gender: "men" }); counts.inserted_teams = (counts.inserted_teams ?? 0) + 1; } else counts.mapped_teams = (counts.mapped_teams ?? 0) + 1; if (value(row, "country").toUpperCase() !== "NEPAL" && leagueId && federationId) contexts.upsertClub({ clubId, leagueId, federationId, countryId: country(value(row, "country")), reputation: 40, financialBand: "MEDIUM", academyStrength: 35, scoutingReach: 50, recruitmentRegions: ["WIDER_ASIA"], simulationDepth: "CONTEXT_ONLY" }); }
    for (const row of plan.players) { const external = value(row, "player_external_id"), imported = importPerson(db, external, value(row, "full_name"), value(row, "date_of_birth"), value(row, "nationality")); persons.set(external, imported.personId);
    /*
     * importPerson reconciles by (name, date of birth, nationality) before
     * ever creating a new person, so a global-dataset entry that happens to
     * be the SAME real individual as an already-imported Nepal-registry
     * player is correctly mapped onto that one person_id — but
     * player_factual_profiles.canonical_external_id is independently unique,
     * so nothing stopped this insert from adding a SECOND, competing profile
     * row for that same person under the global external id, asserting a
     * different (and here, wrong) current club. A person already known under
     * a different external id keeps their existing profile as sole source;
     * this global row is skipped rather than fielding two contradictory
     * identities for one physical player.
     */
    const hasOtherProfile = Boolean(
      db.prepare("SELECT 1 FROM player_factual_profiles WHERE player_id=? AND canonical_external_id<>? LIMIT 1").get(imported.personId, external),
    );
    if (!hasOtherProfile) {
    const clubExternal = clubNames.get(key(value(row, "current_club"))) ?? value(row, "current_club"), clubId = clubs.get(clubExternal) ?? (clubExternal ? (db.prepare("SELECT id FROM clubs WHERE lower(official_name)=lower(?) LIMIT 1").get(clubExternal) as { id?: EntityId } | undefined)?.id : undefined); const profileId = createStableEntityId("import-player-factual", external); db.prepare("INSERT INTO player_factual_profiles (id,player_id,canonical_external_id,current_club_id,factual_json,simulation_json,evidence_json,record_status,confidence_level,last_verified) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(canonical_external_id) DO UPDATE SET current_club_id=excluded.current_club_id,factual_json=excluded.factual_json,evidence_json=excluded.evidence_json,record_status=excluded.record_status,confidence_level=excluded.confidence_level,last_verified=excluded.last_verified").run(profileId, imported.personId, external, clubId ?? null, JSON.stringify(row), JSON.stringify({}), JSON.stringify({ sourceIds: [value(row, "source_1_id"), value(row, "source_2_id")].filter(Boolean) }), value(row, "provenance") || "UNKNOWN", value(row, "confidence") || "LOW", value(row, "last_verified_date") || null); if (clubId && value(row, "nationality").toUpperCase() !== "NEPAL") { contexts.upsertPlayer({ playerId: imported.personId, clubId, region: "WIDER_ASIA", reputation: 30, interestLevel: "UNKNOWN", careerState: "ACTIVE", updatedOn: "2026-08-27" }); const teamId = teams.get(clubExternal); if (teamId) db.prepare("INSERT OR IGNORE INTO team_person_assignments (id,person_id,team_id,role,started_on,ended_on) VALUES (?,?,?,?,?,NULL)").run(createStableEntityId("import-player-assignment", external), imported.personId, teamId, "PLAYER", "2026-08-27"); }
    } /*
