...
import { runScoutingDiagnostic } from "./scouting.js";

const invocationCwd = process.env.INIT_CWD ?? process.cwd();
const args = parseArgs(process.argv.slice(2));
const seed = args.seed ?? "nepal-scouting-diagnostic";
...
const savePath = resolve(
  invocationCwd,
  args["save-path"] ?? join(tmpdir(), `nepal-scouting-${seed}-${process.pid}.sqlite`),
);
...
if (!existsSync(savePath)) {
  createNepalSave({
    ...
    saveName: "Nepal Scouting Diagnostic",
    gameVersion: "0.2.0",
    randomSeed: seed,
  });
}
...
const report = runScoutingDiagnostic(db, {
  clubKey: args.club,
  seed,
  worldDate,
});
