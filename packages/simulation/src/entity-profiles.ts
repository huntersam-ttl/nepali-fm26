...
export const buildClubProfile = (db: GameDatabase, clubId: EntityId, role: CareerRole): ClubProfile => {
  ...
  const owner = db.prepare("SELECT person_id FROM club_ownership_stakes WHERE club_id=? AND controlling_owner=1 ORDER BY percentage DESC,person_id LIMIT 1").get(clubId) as { person_id: EntityId } | undefined;
  ...
  return {
    ...
    owner: owner ? buildEntityReference(db, "INVESTOR", owner.person_id, role) : undefined,
...
export const buildStaffProfile = (db: GameDatabase, personId: EntityId, role: CareerRole): StaffProfileReadModel => {
  ...
  const appointment = db.prepare("SELECT role,club_id,contract_end FROM staff_appointments WHERE person_id=? AND employment_status='ACTIVE' ORDER BY contract_end DESC LIMIT 1").get(personId) as { role?: string; club_id?: EntityId; contract_end?: string } | undefined;
  return {
    ...
    contractEnd: appointment?.contract_end,
    careerHistory: [],
  };
};
...
export const buildCompetitionProfile = (db: GameDatabase, competitionId: EntityId, role: CareerRole): CompetitionProfile => {
  ...
  const standings = season ? repository.standings(season.id).map((row) => ({ team: buildEntityReference(db, "CLUB", row.teamId, role), played: row.played, points: row.points, goalDifference: row.goalDifference })) : [];
