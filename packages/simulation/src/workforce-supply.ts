...
const playableNepalLeagueClubs = (
  ...
  `,
      ...
      .all() as Array<{ club_id: EntityId; team_id: EntityId; division: "A" | "B" | "C" }>
  ).map((row) => ({ clubId: row.club_id, teamId: row.team_id, division: row.division }));
...
const scheduledFixturePeak = (db: GameDatabase, fromDate: string, toDate: string): number => {
  const rows = db
    .prepare(
      "SELECT scheduled_date, COUNT(*) AS count FROM fixtures WHERE scheduled_date >= ? AND scheduled_date <= ? GROUP BY scheduled_date ORDER BY scheduled_date",
    )
...
export const computeWorkforceDemand = (input: {
  ...
  const refereesRequired = Math.max(
    6,
    Math.ceil(fixtures / MAX_APPOINTMENTS_PER_OFFICIAL),
    Math.ceil(peakFixtures * 1.15),
  );
  ...
  for (const club of playableNepalLeagueClubs(db)) {
    ...
    if (!alreadyRun) {
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        if (
          active >= target &&
          Number(positions.goalkeepers ?? 0) > 0 &&
          Number(positions.defenders ?? 0) > 0 &&
          Number(positions.midfielders ?? 0) > 0 &&
          Number(positions.attackers ?? 0) > 0
        )
          break;
        const before = scalar(
          db,
          "SELECT COUNT(*) AS n FROM generated_player_origins g JOIN team_person_assignments tpa ON tpa.person_id=g.player_id WHERE tpa.team_id=? AND tpa.role='PLAYER' AND tpa.ended_on IS NULL",
          club.teamId,
        );
        const report = generateYouthCohort({
          db,
          countryId,
          clubId: club.clubId,
          teamId: club.teamId,
        ...
        const unassigned = db
          .prepare(
            "SELECT g.player_id FROM generated_player_origins g LEFT JOIN team_person_assignments tpa ON tpa.person_id=g.player_id AND tpa.role='PLAYER' AND tpa.ended_on IS NULL WHERE g.club_id=? AND g.generated_on=? AND tpa.person_id IS NULL ORDER BY g.player_id",
          )
          .all(club.clubId, input.date) as Array<{ player_id: EntityId }>;
        for (const player of unassigned)
          world.insertTeamPersonAssignment({
            id: createStableEntityId(
              "lower-league-bootstrap-assignment",
              `${player.player_id}:${club.teamId}`,
            ),
            personId: player.player_id,
            teamId: club.teamId,
            role: "PLAYER",
      ...
      for (const fallback of positionalFallbacks) {
        const covered = scalar(
          db,
          `SELECT COUNT(*) AS n FROM player_attributes pa JOIN team_person_assignments tpa ON tpa.person_id=pa.person_id JOIN generated_player_origins gpo ON gpo.player_id=pa.person_id WHERE tpa.team_id=? AND tpa.role='PLAYER' AND tpa.ended_on IS NULL AND ${fallback.predicate}`,
          club.teamId,
        );
        if (covered > 0) continue;
        const excluded =
          usedFallbackPlayers.length > 0
            ? `AND pa.person_id NOT IN (${usedFallbackPlayers.map(() => "?").join(",")})`
            : "";
        const candidate = db
          .prepare(
            `SELECT pa.person_id FROM player_attributes pa JOIN team_person_assignments tpa ON tpa.person_id=pa.person_id JOIN generated_player_origins gpo ON gpo.player_id=pa.person_id WHERE tpa.team_id=? AND tpa.role='PLAYER' AND tpa.ended_on IS NULL ${excluded} ORDER BY pa.person_id LIMIT 1`,
          )
          .get(club.teamId, ...usedFallbackPlayers) as { person_id?: EntityId } | undefined;
        if (candidate?.person_id) {
          db.prepare("UPDATE player_attributes SET primary_position=? WHERE person_id=?").run(
            fallback.position,
            candidate.person_id,
          );
      ...
      db.prepare("INSERT INTO lower_league_bootstrap VALUES (?,?,?,?)").run(
        club.clubId,
        input.date,
        generated,
        "SIMULATION_ONLY",
    ...
    if (
      !db
        .prepare(
          "SELECT 1 FROM staff_appointments WHERE club_id=? AND role='HEAD_COACH' AND employment_status='ACTIVE' LIMIT 1",
        )
        .get(club.clubId) &&
      !db
        .prepare(
          "SELECT 1 FROM staff_vacancies WHERE club_id=? AND role='HEAD_COACH' AND status='VACANT' LIMIT 1",
        )
        .get(club.clubId)
    ) {
      world.insertStaffVacancy({
        id: createStableEntityId("lower-league-head-coach-vacancy", club.clubId),
    ...
    const playerCounts = db
      .prepare(
        `SELECT COUNT(DISTINCT CASE WHEN gpo.player_id IS NULL THEN tpa.person_id END) AS real_players, COUNT(DISTINCT gpo.player_id) AS generated_players FROM team_person_assignments tpa LEFT JOIN generated_player_origins gpo ON gpo.player_id=tpa.person_id WHERE tpa.team_id=? AND tpa.role='PLAYER' AND tpa.ended_on IS NULL`,
      )
      .get(club.teamId) as { real_players?: number; generated_players?: number };
    const staffCounts = db
      .prepare(
        `SELECT COUNT(DISTINCT CASE WHEN ssp.person_id IS NULL THEN sa.person_id END) AS real_staff, COUNT(DISTINCT ssp.person_id) AS generated_staff FROM staff_appointments sa LEFT JOIN staff_simulation_profiles ssp ON ssp.person_id=sa.person_id WHERE sa.club_id=? AND sa.employment_status='ACTIVE'`,
      )
      .get(club.clubId) as { real_staff?: number; generated_staff?: number };
    const positions = db
      .prepare(
        "SELECT COUNT(*) AS total, SUM(CASE WHEN pa.primary_position='GK' THEN 1 ELSE 0 END) AS goalkeepers, SUM(CASE WHEN pa.primary_position IN ('CB','RB','LB') THEN 1 ELSE 0 END) AS defenders, SUM(CASE WHEN pa.primary_position IN ('DM','CM','AM') THEN 1 ELSE 0 END) AS midfielders, SUM(CASE WHEN pa.primary_position IN ('RW','LW','ST') THEN 1 ELSE 0 END) AS attackers FROM player_attributes pa JOIN team_person_assignments tpa ON tpa.person_id=pa.person_id WHERE tpa.team_id=? AND tpa.role='PLAYER' AND tpa.ended_on IS NULL",
      )
    ...
    results.push({
      ...
      realPlayers: Number(playerCounts.real_players ?? 0),
      generatedStartingPlayers: Number(playerCounts.generated_players ?? 0),
      realStaff: Number(staffCounts.real_staff ?? 0),
      generatedStaff: Number(staffCounts.generated_staff ?? 0),
      playableSquad:
        Number(positions.total ?? 0) >= 11 &&
        Number(positions.goalkeepers ?? 0) > 0 &&
        Number(positions.defenders ?? 0) > 0 &&
        Number(positions.midfielders ?? 0) > 0 &&
        Number(positions.attackers ?? 0) > 0,
...
export const lowerLeagueCoverageReport = (input: {
  db: GameDatabase;
  date: string;
}): LowerLeagueCoverageReport => {
  const clubs = ensureLowerLeaguePlayableWorld({
    db: input.db,
    date: input.date,
    seed: `coverage:${input.date}`,
  });
  const db = input.db;
  const realPlayerTotal = scalar(
    db,
    "SELECT COUNT(DISTINCT pr.person_id) AS n FROM person_roles pr LEFT JOIN generated_player_origins gpo ON gpo.player_id=pr.person_id WHERE pr.role='PLAYER' AND pr.active_to IS NULL AND gpo.player_id IS NULL",
  );
  const freeAgents = scalar(
    db,
    "SELECT COUNT(DISTINCT pr.person_id) AS n FROM person_roles pr LEFT JOIN team_person_assignments tpa ON tpa.person_id=pr.person_id AND tpa.role='PLAYER' AND tpa.ended_on IS NULL WHERE pr.role='PLAYER' AND pr.active_to IS NULL AND tpa.person_id IS NULL",
  );
  const unknownClub = scalar(
    db,
    "SELECT COUNT(DISTINCT pr.person_id) AS n FROM person_roles pr LEFT JOIN team_person_assignments tpa ON tpa.person_id=pr.person_id AND tpa.role='PLAYER' AND tpa.ended_on IS NULL LEFT JOIN generated_player_origins gpo ON gpo.player_id=pr.person_id WHERE pr.role='PLAYER' AND pr.active_to IS NULL AND gpo.player_id IS NULL AND tpa.person_id IS NULL",
  );
  const realStaff = scalar(
    db,
    "SELECT COUNT(DISTINCT sa.person_id) AS n FROM staff_appointments sa LEFT JOIN staff_simulation_profiles ssp ON ssp.person_id=sa.person_id WHERE sa.employment_status='ACTIVE' AND ssp.person_id IS NULL",
  );
  const aDivisionRealPlayers = clubs
    .filter((club) => club.division === "A")
    .reduce((sum, club) => sum + club.realPlayers, 0);
  const bDivisionRealPlayers = clubs
    .filter((club) => club.division === "B")
    .reduce((sum, club) => sum + club.realPlayers, 0);
  const cDivisionRealPlayers = clubs
    .filter((club) => club.division === "C")
    .reduce((sum, club) => sum + club.realPlayers, 0);
  ...
  return {
    generatedOn: input.date,
    realPlayers: realPlayerTotal,
    aDivisionRealPlayers,
    bDivisionRealPlayers,
    cDivisionRealPlayers,
    generatedStartingPlayers: clubs.reduce((sum, club) => sum + club.generatedStartingPlayers, 0),
    freeAgents,
    unknownClub,
    realStaff,
    aDivisionHeadCoaches,
    bDivisionHeadCoaches,
    cDivisionHeadCoaches,
    generatedLowerLeagueManagers,
    clubs,
    provenanceStatus: "SIMULATION_ONLY",
  };
};
