  ...
  for (const club of lowerLeagueClubs(db)) {
    ...
    if (!alreadyRun) {
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        const report = generateYouthCohort({ db, countryId, clubId: club.clubId, teamId: club.teamId, date: input.date, seasonLabel: input.date.slice(0, 4), seed: `${input.seed}:lower:${club.clubId}:${attempt}`, count: Math.min(8, target - active), cohortKey: `lower-league-bootstrap:${club.clubId}:${attempt}`, source: "BOOTSTRAP_SQUAD_REPAIR" });
