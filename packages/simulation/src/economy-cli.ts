...
import { runChairmanDemo, runEconomyDiagnostic } from "./club-economy.js";

const invocationCwd = process.env.INIT_CWD ?? process.cwd();
const args = parseArgs(process.argv.slice(2));
const seed = args.seed ?? "nepal-economy-diagnostic";
...
const savePath = resolve(
  invocationCwd,
  args["save-path"] ?? join(tmpdir(), `nepal-economy-${seed}-${process.pid}.sqlite`),
);
...
if (!existsSync(savePath)) {
  createNepalSave({
    ...
    saveName: "Nepal Economy Diagnostic",
    gameVersion: "0.2.0",
    randomSeed: seed,
  });
}
...
const report =
  args.demo === "chairman"
    ? runChairmanDemo({ db, seed, worldDate: startDate, clubId: args.club as any })
    : runEconomyDiagnostic({ db, seed, startDate, seasons });
db.close();
