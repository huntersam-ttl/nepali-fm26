...
const managerEvent = (event: HistoricalEvent): boolean =>
  numericImportance(event) >= 5 &&
  /(PROMISE|TRANSFER|LOAN|INJURY|MANAGER|MATCH|PROMOTION|RELEGATION|TROPHY|CHAMPION|PROTEST|SUPPORTER|CONCERN_ESCALATED|DEMAND|CAPTAINCY|TEAM_MEETING)/i.test(
    event.eventType,
  );
