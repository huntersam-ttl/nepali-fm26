...
import { NEPAL_GEOGRAPHY, nepalPack, startingClubOptions } from "@nepal-football-sim/simulation";
...
describe("starting-club locality fallback", () => {
  ...
  it("gives every starting-club option a defined, non-'unknown' location label", () => {
    const options = startingClubOptions(dataset, nepalPack);
  ...
  it("is deterministic — the same club always gets the same estimate across calls", () => {
    const first = startingClubOptions(dataset, nepalPack).map((option) => [option.clubName, option.locationName]);
    const second = startingClubOptions(dataset, nepalPack).map((option) => [option.clubName, option.locationName]);
