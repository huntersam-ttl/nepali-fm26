...
describe("club licensing production activation", () => {
  ...
  it("keeps a target division full when a sporting promotion fails licensing", () => {
    ...
    const standings = [{ competitionSeasonId: source.id, teamId: "blocked-team" as EntityId, played: 1, won: 1, drawn: 0, lost: 0, goalsFor: 1, goalsAgainst: 0, goalDifference: 1, points: 3 }, { competitionSeasonId: source.id, teamId: "eligible-team" as EntityId, played: 1, won: 1, drawn: 0, lost: 0, goalsFor: 1, goalsAgainst: 0, goalDifference: 1, points: 3 }] as LeagueStanding[];
    const result = progressPyramidSeason({ completedSeasons: [{ season: source as CompetitionSeason, ruleSet: rule as CompetitionRuleSet, standings, memberships: [membership("blocked", "blocked-team"), membership("eligible", "eligible-team")] }], nextSeasons: new Map([[target.competitionId, target as CompetitionSeason]]), relationships: [{ id: "promotion" as EntityId, fromCompetitionId: source.competitionId, toCompetitionId: target.competitionId, movementType: "PROMOTION", numberOfTeams: 1, selectionMethod: "TOP_TABLE" } as CompetitionRelationship], eligibleClubIds: new Set(["eligible" as EntityId]) });
