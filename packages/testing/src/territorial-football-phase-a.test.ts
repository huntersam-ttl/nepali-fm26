...
import { createDistrictDevelopmentProject, initializeTerritorialStructure, updateDistrictDevelopment } from "@nepal-football-sim/simulation";
...
describe("Nepal territorial football phase A", () => {
  it("initialises all seven provinces and 77 districts deterministically", () => {
    ...
    expect(initializeTerritorialStructure(db, "2026-08-01").districts.map((district) => district.id)).toEqual(first.districts.map((district) => district.id)); db.close();
  });

  it("keeps development bounded and records a project lifecycle state", () => {
    const db = openGameDatabase(":memory:"); migrateDatabase(db); const { districts } = initializeTerritorialStructure(db, "2026-08-01"); const district = districts[0]!;
