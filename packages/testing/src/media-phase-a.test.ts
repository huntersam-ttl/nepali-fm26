...
describe("Media phase A", () => {
  it("publishes deterministic high-importance save-state events and inbox stories", () => {
    ...
    new EventRepository(first).insertHistoricalEvent(event); new EventRepository(second).insertHistoricalEvent(event);
    const lowEvent: HistoricalEvent = { id: createStableEntityId("history", "media-low-event"), occurredOn: "2026-09-01", eventType: "ROUTINE_TRAINING", involvedEntities: [], title: "Routine training completed", importance: "low", scope: "club" };
    new EventRepository(first).insertHistoricalEvent(lowEvent);
    initializeMediaForSave(first); initializeMediaForSave(second);
