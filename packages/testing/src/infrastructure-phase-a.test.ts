...
const makeSave = (seed: string): string => {
  const dir = mkdtempSync(join(tmpdir(), "infrastructure-phase-a-")); dirs.push(dir);
...
describe("infrastructure and institution building phase A", () => {
  it("keeps unfunded projects in financing and preserves shared-site rights", () => {
    ...
    const project = createInfrastructureProject(db, { clubId: club.id, projectType: "TRAINING_GROUND", date: "2026-08-01", seed: "infrastructure-lifecycle", financing: { clubCash: 0.2 }, siteRights: "SHARED" });
    advanceInfrastructureProjects(db, { date: "2026-10-01", seed: "infrastructure-lifecycle" });
    expect(new ClubEconomyRepository(db).infrastructureProjects(club.id).find((item) => item.id === project.id)?.status).toBe("FINANCING");
    db.close();
  });
  ...
  it("completes funded projects deterministically, updates facilities and records maintenance", () => {
    ...
    initializeClubEconomyForSave({ db: first, worldDate: "2026-08-01", seed: "infrastructure-complete" });
    initializeClubEconomyForSave({ db: second, worldDate: "2026-08-01", seed: "infrastructure-complete" });
    const club = first.prepare("SELECT id FROM clubs WHERE name = ?").get("Machhindra FC") as { id: EntityId };
    const a = createInfrastructureProject(first, { clubId: club.id, projectType: "MEDICAL_ROOM", date: "2026-08-01", seed: "infrastructure-complete" });
    const b = createInfrastructureProject(second, { clubId: club.id, projectType: "MEDICAL_ROOM", date: "2026-08-01", seed: "infrastructure-complete" });
    expect({ cost: a.capitalCost, completion: a.expectedCompletion }).toEqual({ cost: b.capitalCost, completion: b.expectedCompletion });
    ...
    expect(economy.infrastructureProjects(club.id).find((item) => item.id === a.id)?.status).toBe("COMPLETED");
