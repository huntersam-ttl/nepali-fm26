...
const ChairmanDashboardView = ({ dashboard, header, roles, bridge, refresh, candidates, refreshCandidates }: { dashboard: ChairmanDashboard; header: CareerHeader; roles: CareerRoleState; bridge: DesktopRuntimeApi; refresh: () => void; candidates: OwnerManagerCandidate[]; refreshCandidates: () => void }): React.ReactElement => {
  ...
  const saveBudget = async (): Promise<void> => { if (!budget || busyBudget) return; setBusyBudget(true); const result = await bridge.setClubBudget(dashboard.club.id, budget.seasonLabel, budget.category, Number(amount)); setBusyBudget(false); if (result.ok) { setMessage("Budget updated."); setError(null); refresh(); } else setError(result.error); };
  const orderEquipment = async (): Promise<void> => { if (busyEquipment) return; setBusyEquipment(true); const result = await bridge.purchaseEquipment("FOOTBALL_EQUIPMENT", 1); setBusyEquipment(false); if (result.ok) { setMessage("Equipment order placed."); setError(null); refresh(); } else setError(result.error); };
