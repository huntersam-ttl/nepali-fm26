 ...
 * (Phase 6D). Runs on its own port (1422) so it never touches the ambient
 * dev server on 1420 that another session may own, and uses an isolated
 * saves directory so it cannot collide with any shared/ambient save. Not
 * part of the committed config.
 */
...
export default defineConfig({
  ...
  testMatch: /decision-presentation\.spec\.ts/,
  timeout: 180_000,
